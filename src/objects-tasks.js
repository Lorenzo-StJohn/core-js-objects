...
function shallowCopy(obj) {
  ...
  if (Array.isArray(obj)) {
    ans = [];
    obj.forEach((el) => {
      if (typeof el === 'object') ans.push(shallowCopy(el));
      else ans.push(el);
    });
    return ans;
  }
  ans = {};
  ans = Object.assign(ans, obj);
  const keys = Object.keys(obj);
  keys.forEach((key) => {
    if (typeof obj[key] === 'object') ans[key] = shallowCopy(obj[key]);
  });
  return ans;
}
...
function mergeObjects(objects) {
  ...
  objects.forEach((object) => {
    ...
    entries.forEach(([key, value]) => {
      if (ans[key] === undefined) ans[key] = value;
      else ans[key] += value;
    });
  });
  return ans;
...
function compareObjects(obj1, obj2) {
  ...
  key1.forEach((key, index) => {
    if (key !== key2[index]) ans = false;
    if (obj1[key] !== obj2[key2[index]]) ans = false;
  });
  return ans;
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((obj) => {
    ...
    const arr = ans.get(key) ? ans.get(key) : [];
    arr.push(value);
    ans.set(key, arr);
  });
  return ans;
